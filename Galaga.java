...
class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    void spawnLevel() {
        ...
        explosions.clear();
        enemyDir = 1;
        enemyMoveInterval = Math.max(10, 40 - (level - 1) * 5);
        entranceTimer = 180;
        px = W / 2 - 20;
    ...
    void update() {
        ...
        if (enemyDiveTimer >= enemyDiveCooldown && !enemies.isEmpty()) {
            ...
            // Pick a random non-diving enemy to dive
    ...
    void drawEnemyType(Graphics2D g2, int x, int y, int type, boolean frame2) {
        if (type == 2) {
            // Boss Galaga – butterfly style, yellow/orange
            drawBossEnemy(g2, x, y, frame2);
        } else if (type == 1) {
            // Mid-tier – blue/cyan bee
...
class Enemy {
    ...
    void startDive(int px, int py) {
        diving = true;
        divePhase = 0;
        // Angle toward player with some randomness
